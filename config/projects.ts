...
export const projects: Project[] = [
{
    ...
    status: "shipped",
    blurb:
      "A caching reverse proxy in C on a Raspberry Pi 5, benchmarked four ways — single-threaded, thread-per-connection, thread pool, and an epoll event loop — to find where 'fastest' stops being the right question.",
    stack: ["C", "HTTP", "Raspberry Pi", "Sockets", "epoll", "Concurrency", "Benchmarking"],
    images: [
    ...
    body: [
      "I wanted to understand the layer between a browser and a server that CDNs and reverse proxies quietly handle, so I built one from scratch in C: an HTTP caching proxy that caches responses in memory (hash table with separate chaining, O(1) lookup; LRU eviction via a doubly-linked recency list, O(1) evict) and forwards misses to an origin. Then I turned it into a study of how the thing actually performs — measure, diagnose, build, re-measure, tune.",
      "The headline first: cache hits served ~29× the throughput of origin fetches, because a hit is a pure in-memory lookup while a miss pays a full round trip. Then I stress-tested my assumptions. LFU eviction collapsed the hit rate under drifting popularity (~9.5% vs LRU's ~75%) — old frequency counts become permanent baggage. And I hardened it: allocation-failure safety, socket timeouts, truncation-safe caching against cache poisoning.",
      "Then I compared four concurrency architectures head-to-head, all recompiled under identical conditions for apples-to-apples numbers: single-threaded, thread-per-connection, a thread pool (with a mutex/condition-variable job queue, tuned via a worker-count sweep to ~16), and finally a single-threaded epoll event loop over non-blocking sockets — the way nginx works, driving both client and origin connections through one state machine so no thread ever blocks on I/O.",
      "The most important thing I learned is that there is no single 'fastest' architecture — and that raw throughput is a misleading metric. Single-threaded actually won peak throughput at low concurrency (zero coordination overhead), but that number hid catastrophic tail latency: a 1.7-second worst case and timeouts under load, because a few requests scream through while others starve. The thread pool and event loop trade a little peak throughput for enormous stability — sub-millisecond average latency and single-digit-millisecond worst case at the same load. epoll gave the best overall balance and the best miss-path throughput, with one thread and no per-connection cost.",
      "That's why production servers like nginx use an event loop: at scale, predictable latency matters more than peak throughput. The lesson wasn't 'epoll is fastest' — it was that 'fastest' is the wrong question. Tail latency and stability under load are what separate these designs, and you only see it when you look past the headline number. Working this close to the metal in C, and measuring every change on real hardware, taught me the systems-performance reasoning the higher-level tools hide.",
    ],
  },
  {
    slug: "ecotrack",
